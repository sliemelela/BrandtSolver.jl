using Test
using BrandtSolver
using FinancialMarketSimulation
using CairoMakie
using LinearAlgebra
using Statistics
using StaticArrays
using Random

include("../test_helpers.jl")

OUTPUT_DIR = joinpath(@__DIR__, "..", "output")
if !isdir(OUTPUT_DIR)
    mkdir(OUTPUT_DIR)
end

println("\n--- Visual Verification: Double Stock Portfolio ---")


function setup_bond_market_merton_stock()
    # 1. Interest Rate (Vasicek)
    # Volatile enough to make the bond risky and hedging interesting.
    # Mean 5%, Vol 2%, Speed 0.2
    r_proc = VasicekProcess(:r, 0.2, 0.05, 0.02, 0.05, 1)

    # 2. Stock (Merton-Style Drift)
    # Target: dS/S = (r_t + λσ)dt + σdW
    # Excess Return = (r_t + λσ) - r_t = λσ (Constant!)
    # This isolates Hedging Demand from Speculative Demand.
    σ_S = 0.20
    risk_premium = 0.05

    # Note: We now pass r_val into the drift function
    stock_drift(t, S, r_val) = S * (r_val + risk_premium)
    stock_diff(t, S, r_val)  = S * σ_S

    # IMPORTANT: We add [:r] to dependencies so the process reads the rate path
    stock_proc = GenericSDEProcess(:S, stock_drift, stock_diff, 100.0, [2], [:r])

    # 3. Second stock (Merton-Style Drift, Different Risk Premium and Vol)
    stock_drift_2(t, S, r_val) = S * (r_val + 0.02)
    stock_diff_2(t, S, r_val) = S * 0.1
    stock_proc_2 = GenericSDEProcess(:S_2, stock_drift_2, stock_diff_2, 100.0, [3], [:r])

    #
    market_risk = [-0.2, -0.1, -0.15]

    # 4. Excess Returns
    re_stock = ExcessReturnProcess(:Re_Stock, :S, :r)
    re_stock_2  = ExcessReturnProcess(:Re_Stock_2, :S_2, :r)


    # 5. Config
    # Correlation between Rate (1) and Stock (2)
    ρ = [ 1.0 -0.4 -0.6;
         -0.4  1.0  0.3;
         -0.6  0.3  1.0]

    config = MarketConfig(
        sims = 2000,
        T = 5.0,
        M = 10,
        processes = [r_proc, stock_proc, stock_proc_2, re_stock, re_stock_2],
        correlations = ρ
    )

    world = build_world(config)

    # 6. Solver Params
    params = SolverParams(
        W_grid = [50.0, 100.0, 150.0, 200.0],
        poly_order = 2,
        max_taylor_order = 4,
        trimming_α = 0.01,
    )

    # 7. Utility
    γ = 5.0
    crra(W) = (W^(1.0 - γ)) / (1.0 - γ)
    utility = create_utility_from_ad(crra)

    return world, params, utility
end

@testset "Visual Double Stock Diagnostics" begin
    Random.seed!(999)

    println("  > Running Solver...")
    world, params, utility = setup_bond_market_merton_stock()

    asset_names = ["Re_Stock", "Re_Stock_2"]
    state_names = ["r"]

    # Extract matrices
    Re_all = package_excess_returns(world, asset_names)
    Z_all  = get_state_variables(world, state_names)
    X_all, Y_all = create_risk_free_return_components(world, 0.0, nothing)

    # Run decoupled solver
    policies = solve_portfolio_problem(Re_all, Z_all, X_all, Y_all, params, utility)

    println("  > Generating Plots...")

    fig1 = plot_policy_rules(policies, params, 10, asset_names; samples=20)
    save(joinpath(OUTPUT_DIR, "double_stock_1_policy_vs_wealth.png"), fig1)

    state_vals = world.paths.r[:, 10]
    fig2 = plot_state_dependence(policies, params, 9, state_vals, "r", asset_names; fix_W=100.0)
    save(joinpath(OUTPUT_DIR, "double_stock_2_policy_vs_rate.png"), fig2)

    times = range(0, world.config.T, length=world.config.M+1)[1:end-1]
    fig3 = plot_realized_weights(Re_all, X_all, Y_all, times, policies, asset_names; W_init=100.0)
    save(joinpath(OUTPUT_DIR, "double_stock_3_realized_weights.png"), fig3)

    fig4 = plot_value_vs_utility(Re_all, X_all, Y_all, params, policies, utility; t_check=world.config.M)
    save(joinpath(OUTPUT_DIR, "double_stock_4_value_vs_utility.png"), fig4)

    fig5 = plot_policy_surface(policies, params, 10, state_vals, "r", asset_names)
    save(joinpath(OUTPUT_DIR, "double_stock_5_policy_surface.png"), fig5)

    println("--- Visual Double Stock Test Complete ---")
end